#!/usr/bin/env python3
"""
Train quantity badge classifier and export ONNX.
Classes expected:
- none
- equip
- 01..24
Dataset layout:
  data/quantity_dataset/
    none/
    equip/
    01/
    02/
...
def main():
    ...
    p.add_argument('--out', type=Path, default=Path('public/models/artifacts/quantity-classifier.onnx'))
    p.add_argument('--epochs', type=int, default=6)
    p.add_argument('--batch', type=int, default=64)
    p.add_argument('--model-name', type=str, default='quantity')
    args = p.parse_args()
    if args.epochs <= 0:
    ...
    train_ds = datasets.ImageFolder(args.data, transform=tf)
    ...
    labels_out = args.out.with_name("quantity-labels.json")
    labels_out.write_text(json.dumps(class_values, indent=2))
    print(f"[train:{args.model_name}] exported: {labels_out}", flush=True)

#!/usr/bin/env python3
"""
Train diamonds classifier and export ONNX.
Classes expected: 0..3 where class value equals number of diamonds.
Dataset layout:
...
def main():
    ...
    p.add_argument('--out', type=Path, default=Path('public/models/artifacts/diamonds-classifier.onnx'))
    p.add_argument('--epochs', type=int, default=4)
    p.add_argument('--batch', type=int, default=64)
    p.add_argument('--model-name', type=str, default='diamonds')
    args = p.parse_args()
    if args.epochs <= 0:
    ...
    if n_classes < 4:
        raise RuntimeError(f'Expected at least 4 diamond classes, got {n_classes}')
